...
bool bubble_sort_test(int a[], const int a_expected[], const std::size_t n)
{
    bubble_sort(a, n);
    return memcmp(a, a_expected, n * sizeof(a[0])) == 0;
}

TEST(bubble_sort_test, 1)
{
    const std::size_t n = 3;
    int a[n] = {7, 5, 6};
    int e[n] = {5, 6, 7}; // E - stands for Expected after sort
    EXPECT_TRUE(bubble_sort_test(a, e, n));
}

TEST(bubble_sort_test, 2)
{

...
bool quick_sort_test(int a[], const int e[], const std::size_t n)
{
    quick_sort(a, 0, n - 1);
    return memcmp(a, e, n * sizeof(a[0])) == 0;
}

TEST(quick_sort_test, 1)
{
    const std::size_t n = 3;
    int a[n] = {7, 5, 6};
    int e[n] = {5, 6, 7}; // E - stands for Expected after sort
    EXPECT_TRUE(quick_sort_test(a, e, n));
}

TEST(quick_sort_test, 2)
{

...
bool insertion_sort_test(int a[], const int e[], const std::size_t n)
{
    insertion_sort(a, n);
    return memcmp(a, e, n * sizeof(a[0])) == 0;
}

TEST(insertion_sort_test, 1)
{
    const std::size_t n = 3;
    int a[n] = {7, 5, 6};
    int e[n] = {5, 6, 7}; // E - stands for Expected after sort
    EXPECT_TRUE(insertion_sort_test(a, e, n));
}

TEST(insertion_sort_test, 2)
{
